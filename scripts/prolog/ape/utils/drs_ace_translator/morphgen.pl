...
pos_comp(Positive, [Comparative], []) :-
	adj_itr_comp(Comparative, Positive),
	!.

pos_comp(Positive, [Comparative], [Preposition]) :-
	adj_tr_comp(Comparative, Positive, Preposition),
	!.
...
pos_sup(Positive, [Superlative]) :-
	adj_itr_sup(Superlative, Positive),
	!.

pos_sup(Positive, [Superlative]) :-
	adj_tr_sup(Superlative, Positive, _),
	!.
...
surface_noun(cn, Lemma, sg, Form) :-
	noun_sg(Form, Lemma, _),
	!.

surface_noun(cn, Lemma, mass, Form) :-
	noun_mass(Form, Lemma, _),
	!.

surface_noun(cn, Lemma, pl, Form) :-
	noun_pl(Form, Lemma, _),
	!.
...
acesentencelist_pp([X | Xs], Ace) :-
	...
	!,
	maplist_acesentencelist_pp_x([X | Xs], AceList),
	concat_atom(AceList, '\n\n', Ace).

maplist_acesentencelist_pp_x([],[]).

maplist_acesentencelist_pp_x([H1|R1],L):-
	acesentencelist_pp_x(H1,H2),
	L = [H2|R2],
	maplist_acesentencelist_pp_x(R1,R2).
...
sentence_type([qp(QPhrase) | T], _, Tokens) :-
	!,
	sentence_type(T, '?', T2),
	basics:append(QPhrase, T2, Tokens).
