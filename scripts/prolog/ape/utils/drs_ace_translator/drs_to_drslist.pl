...
drs_to_drslist(drs(_, CondList), DrsListSorted) :-
	...
	maplist_condlist_to_drs(CondListList, DrsList),
	sort_drslist(DrsList, DrsListSorted).

maplist_condlist_to_drs([],[]).

maplist_condlist_to_drs([H1|R1],[H2|R2]):-
	condlist_to_drs(H1,H2),
	maplist_condlist_to_drs(R1,R2),
	L = [H2|R2].
...
get_toplevel_conds([[H | T] | Conds], SelectedConds) :-
	...
	basics:append(SelectedConds1, SelectedConds2, SelectedConds).
...
%%	partition(terms_share_variables(Term), TermListList, Included, Excluded),
	partition(Term, TermListList, Included, Excluded),
	append2(Included, IncludedFlat).
	
append2([],[]).
append2([X],X).
append2([X1,X2],X):-basics:append(X1,X2,X).
append2([X1,X2,X3|Rest],L):-append2([X2,X3|Rest],L1),basics:append(X1,L1,L).

partition(Term,[Term1|RestTerms],Included,Excluded):-
	(terms_share_variables(Term,Term1)
	->
	Included = [Term1|RestIncluded],
	Excluded = RestExcluded
	;
	Included = RestIncluded,
	Excluded = [Term1|RestExcluded]
	),
	partition(Term,RestTerms,RestIncluded,RestExcluded).
...
sort_drslist(DrsList, DrsListSorted) :-
	maplist_assign_key(DrsList, DrsListWithKeys),
	keysort(DrsListWithKeys, DrsListWithKeysSorted),
	maplist_remove_key(DrsListWithKeysSorted, DrsListSorted).

maplist_assign_key([],[]).

maplist_assign_key([H1|R1],[H2|R2]):-
	basics:length([H1|R1],Len1),
	basics:length([H2|R2],Len2),
	(Len1 \= Len2
	->
	fail
	;
	...
	fail
	;
	maplist_assign_key(R1,R2)
	).
	
maplist_remove_key([],[]).

maplist_remove_key([H1|R1],[H2|R2]):-
	basics:length([H1|R1],Len1),
	basics:length([H2|R2],Len2),
	(Len1 \= Len2
	->
...
assign_key_to_conds(Conds, Key) :-
	maplist_get_key(Conds, Keys),
	sort(Keys, [Key | _]).
	
maplist_get_key([],[]).

maplist_get_key([H1|R1],[H2|R2]):-
	basics:length([H1|R1],Len1),
	basics:length([H2|R2],Len2),
	(Len1 \= Len2
	->
