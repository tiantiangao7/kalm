...
include_is_complex_or_predicate([],[]).
include_is_complex_or_predicate([H1|R1],L):-
	(is_complex_or_predicate(H1)
	->
	L = [H1|R2]
	;
	L = R2
	),
	include_is_complex_or_predicate(R1,R2).
...
subtract([],_,[]):-!.	
subtract(L,[],L):-!.
subtract([H1|R1],L2,L3):-
	(basics:memberchk(H1,L2)
	->
	L3 = R2
	;
	L3 = [H1|R2]
...
drs_to_coreace_nvar(Drs, BoxId, [ToplevelReferentList, UnsortedSubjectList, UnsortedObjectList, NamedList], RI--RO, Result1) :-
	...
	basics:append(InitialReferents, ThereIsReferents, List3),
...
counter(Name, N) :-
		...
		(getval(Name, N0), writeln('t1'),succ(N0, N),writeln('t2'), setval(Name, N)),
		_,
		(N = 1, setval(Name, N))
	).
...
walk_cond([C | Cs], _BoxId) :-
	include_is_complex_or_predicate([C | Cs], Predicates),
