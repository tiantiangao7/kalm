...
include_is_implication([],[]).
include_is_implication([H1|R1],L):-
	(is_implication(H1)
	->
	L = [H1|R2]
	;
	...
	include_is_implication(R1,R2).


exclude_is_named([],[]).
exclude_is_named([H1|R1],L):-
	(is_named(H1)
	->
	L = R2
	;
	...
	exclude_is_named(R1,R2).

exclude_is_named_ref([],[]).
exclude_is_named_ref([H1|R1],L):-
	(is_named_ref(H1)
	->
	L = R2
	;
...
drs_acetext_np_x(Drs, AceText) :-
	...
	exclude_is_named(DrsSimpleWithNamed, DrsSimple),

	numbervars(DrsSimple, 0, _),
	retractall(ref_to_noun(_, _, _, _, _)),
	clear_vars,
	retractall(name_namespace(_, _)),
	...
	basics:append(UnsortedSubjectList, List1, List2),
	list_to_set(List2, SubjectList),
	subtract(SubjectList, NamedList, UnnamedSubjectListWithNamed),
	% BUG: temporary: remove named-object-conditions
	exclude_is_named_ref(UnnamedSubjectListWithNamed, UnnamedSubjectList),
	...
	exclude_is_named_ref(Remaining2WithNamed, Remaining2),

	individuals_ace(Remaining2, AceTextListIndividuals),

	include_is_implication(DrsSimple, Implications),
	...
	basics:append(AceTextListProperties, AceTextListIndividuals, Tmp),
	basics:append(AceSubjectList, Tmp, Tmp1),
	basics:append(Tmp1, AceTextListClasses, AceText).
...
flatten([L|Ls], FlatL) :-
    ...
    flatten(L, NewL),
    flatten(Ls, NewLs),
    basics:append(NewL, NewLs, FlatL).
flatten(L, [L]).	
