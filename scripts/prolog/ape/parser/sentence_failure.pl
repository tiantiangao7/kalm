...
%% modification
list_to_set(List,Set):-is_list(List),is_list(Set),List = [],Set = [], !.
list_to_set(List,Set):-
	is_list(List),var(Set),!,Set = List.
list_to_set(List,Set):-
	is_list(List),is_list(Set),
	[H1|T1] = List, [H2|T2] = Set,
	H1 = H2, list_to_set(T1,T2).	
...
get_sentence_error_text(SentenceTokens, ErrorText) :-
	member(IllegalWord, SentenceTokens),is_illegalword(IllegalWord, ErrorText).
...
get_sentence_error_text(SentenceTokens, ErrorText) :-
	...
	\+ is_repeatable(Token),
	atom_concat('Token ''',Token,S1), atom_concat(S1,''' repeated.',ErrorText).
...
get_sentence_error_text(SentenceTokens, ErrorText) :-
	...
	atom_concat('The construct ''there is'' + ''',Token,S1),atom_concat(S1,''' is not allowed.',ErrorText).
...
get_sentence_error_text(SentenceTokens, ErrorText) :-
	...
	atom_concat('The construct ''there are'' + ''',Token,S1),atom_concat(S1,''' is not allowed.',ErrorText).
...
get_sentence_error_text(SentenceTokens, ErrorText) :-
	...
	atom_concat('The intransitive verb ''',Verb,S1),atom_concat(S1,''' cannot be followed by that-subordination. Use a transitive verb.',ErrorText).
