...
%:- ensure_loaded('D:\\APE-XSB\\lexicon\\chars').
%:- use_module(chars, [
%		is_letter/1,
%		is_digit/1
%	]).

%:- ensure_loaded('D:\\APE-XSB\\logger\\error_logger').
%:- use_module(error_logger, [
%		add_error_message_once/4,
%		add_warning_message_once/4
%	]).

%:- ensure_loaded('D:\\APE-XSB\\utils\\ace_niceace').
...
% 2. Modifies certain token sequences, e.g.
%
...
get_atomics([C | Cs], Ts) :-
	char_code(CharCode,C),atom(CharCode),
...
string_to_token(Prefix, Ts, [T | Ts]) :-
	atom_codes(S, Prefix),
	atom_concat('"',S,S1),atom_concat(S1,'"',T).
%	concat_atom(['"', S, '"'], T).
