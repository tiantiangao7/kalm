...
%:- use_module(chars, [is_capitalized/1]).
...
spellcheck_and_report_unknown_words(UnknownTokenList, N) :-
	damerau_rules(UnknownTokenList, Candidates, NoSolutions),
	!,
...
known_word(_, _, Prefix, ':') :-
	(
		propername_prefix(Prefix, _)
	;
		noun_prefix(Prefix, _)
	;
		verb_prefix(Prefix)
	;
		modif_prefix(Prefix)
...
%modification
b_setval(npid,NewNPID):-
	assert(gl_var(npid,NewNPID)).

b_setval(npid,NewNPID):-
	retract(gl_var(npid,NewNPID)),fail.

b_getval(npid, OldNPID):- findall(ID,gl_var(npid, ID),L),max_list(L,OldNPID).
