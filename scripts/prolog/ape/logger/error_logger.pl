...
assert_message(Importance, Type, SentenceID-TokenID, Subject, Description) :-
%     format:format("Hello ~q!",world).
     format:format('Malformed message: ~q\n', [message(Importance, Type, SentenceID-TokenID, Subject, Description)]),
     !.
...
setval(Name, Value) :-
    copy_term(Value,Val2), retractall(gl_err_var(Name,_)), assert(gl_err_var(Name,Val2)).
...
getval(Name, Value) :-
    gl_err_var(Name,Val2), copy_term(Val2,Value).
